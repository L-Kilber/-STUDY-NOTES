...
for i in daily_transactions_split:
  transactions_clean.append(i)
for i in range(len(transactions_clean)):
  for j in range(len(transactions_clean[0])):
    transactions_clean[i][j] = transactions_clean[i][j].strip(" ")
...
for i in range(len(transactions_clean)):
  ...
  thread_sold.append(transactions_clean[i][2].strip("\n").strip(" "))

total_sales = 0
for i in range(len(sales)):
  sales[i] = sales[i].strip("\n$")
  sales[i] = sales[i].strip(" ")
  total_sales += float(sales[i])
...
for i in colors:
  print("Today were sold: {count} {color}".format(count=color_count(i), color=i.title()))
